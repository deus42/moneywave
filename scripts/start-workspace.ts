...
const remoteOrigin = process.env.MONEYWAVE_TAILSCALE_ORIGIN;
const remoteLogin = process.env.MONEYWAVE_TAILSCALE_LOGIN;
if (Boolean(remoteOrigin) !== Boolean(remoteLogin)) throw new Error('TAILSCALE_CONFIG_INVALID');
...
const server = createWorkspaceServer({store,centers:new FinanceCenters(database),crypto:new CryptoStore(database),staticDirectory:resolve('src/web'),sourceCurrent,tailscale:remoteOrigin && remoteLogin ? {origin:remoteOrigin,login:remoteLogin} : undefined});
