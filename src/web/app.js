...
function budgetEdit(cat='') {
 ...
 open('Ліміт бюджету',`<form id="budget-form" data-category="${esc(cat)}">${field('Категорія',cat?`<input name="category" value="${esc(categoryName(cat))}" readonly>`:'<input name="category" required maxlength="240">')}${field('Місячний ліміт, EUR',`<input name="amount" type="number" min="0" max="10000000" step="0.01" value="${(budgetValue(cat,from)/100).toFixed(2)}" required>`)}${field('Діє з місяця',`<input name="from" type="month" value="${from}" required>`)}<p class="quiet">Попередні місяці зберігають свій план. Річний ліміт — сума місячних лімітів.</p><div class="drawer-actions"><button class="button">Зберегти ліміт</button><button type="button" class="chip" data-action="close">Скасувати</button></div></form>`);
}
function rowEdit(id) {
