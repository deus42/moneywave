...
const cashflowRow=r=>!r.excluded&&r.reportingScope!=='trip_only';
...
function selectedCollections(kind) {
 ...
 return workspace.state.collections.filter(c=>(kind==='purchase'?c.kind==='purchase':c.kind!=='purchase')&&(!seasonOnly||c.season)&&(collectionKind==='all'||c.kind===collectionKind)&&`${c.name} ${c.note} ${c.start}`.toLocaleLowerCase('uk-UA').includes(collectionQuery.toLocaleLowerCase('uk-UA'))&&(period==='all'||allCollections||(c.datePrecision==='year'?c.start>=from&&c.end<=to:c.start<=to&&c.end>=from)||(c.kind!=='trip'&&totals(c).rows.some(r=>inPeriod(r.date))))).sort((a,b)=>b.start.localeCompare(a.start));
}
...
function collections(kind) {
 ...
 return heading('Покупки','<button class="button" data-new="purchase">+ Покупка</button>')+collectionToolbar('purchase')+(selected.length?`${collectionSummary(selected)}<div class="purchase-list">${selected.map(c=>{const t=totals(c);return `<button class="purchase-row" data-collection="${esc(c.id)}"><span class="purchase-copy"><strong>${esc(c.name)}</strong><span>${esc(tripDates(c))}${c.coverageNote?' · Частина витрат':''}</span></span><span class="purchase-value"><strong>${t.count?euro(t.net,2):'—'}</strong>${t.recovered?`<small>Повернуто ${euro(t.recovered,2)}</small>`:c.budget!==null?`<small class="${t.net>c.budget?'red':''}">Бюджет ${euro(c.budget)}</small>`:''}</span><span class="purchase-arrow" aria-hidden="true">→</span></button>`;}).join('')}</div>`:collectionEmpty('purchase'));
}
function transactionRows(rows,editable=true) {return rows.map(r=>`<tr class="${r.excluded?'excluded-row':''}"><td>${paymentDate(r.date)}</td><td><button ${editable?`data-row="${esc(r.id)}"`:''}>${esc(r.description)}</button><div class="subline">${esc(r.provider)}${r.trip?' · '+esc(r.trip):''}${r.excluded?' · поза витратами':''}</div></td><td>${editable?`<button class="category-badge" data-row="${esc(r.id)}" aria-label="Редагувати: ${esc(r.description)}">${esc(rowCategoryLabel(r))}</button>`:esc(rowCategoryLabel(r))}</td><td class="${r.eur<0?'green':''}">${euro(r.eur,2)}${nativeAmount(r)&&r.currency!=='EUR'?`<span class="subline display-block native-amount">${esc(nativeAmount(r))}</span>`:''}</td></tr>`).join('');}
...
function rowEdit(id) {
 ...
 const r=allRows().find(r=>r.id===id),override=workspace.state.overrides[id];if(!r)return;
 ...
 open('Операція',`<div class="operation-summary"><div class="quiet">${date(r.date)} · ${esc(prettyProvider(r.provider))}</div><div class="operation-amount"><span>${label}</span><strong>${esc(native??euro(Math.abs(r.eur),2))}</strong>${native&&r.currency!=='EUR'&&r.nativeEur==null?`<span>${euro(Math.abs(r.eur),2)} у звіті</span>`:!native?'<span>EUR-оцінка</span>':''}</div></div><form id="row-form" data-id="${esc(id)}">
 ${field('Тип операції',`<select name="operationType" aria-describedby="operation-effect">${Object.entries(operationLabels).filter(([key])=>key!=='cash_fx'||r.eur>0).map(([key,label])=>`<option value="${key}" ${key===type?'selected':''}>${esc(label)}</option>`).join('')}</select>`)}
 <p id="operation-effect" class="quiet operation-effect" aria-live="polite"></p>
 <div id="operation-category">${field('Категорія',`<select name="category">${categoryOptions(override?.category??r.homeGroup??r.group)}</select>`)}${r.trip?`<p class="quiet">У бюджеті поїздки «${esc(r.trip)}».</p>`:''}</div>
 ${field('Назва',`<input name="name" value="${esc(r.description)}" maxlength="4000" required>`)}
 <details class="form-detail"><summary>Нотатка</summary>${field('Нотатка',`<textarea name="note" rows="2" maxlength="2000">${esc(override?.name!==undefined?override.note:'')}</textarea>`)}</details>
...
function updateOperationForm(){
 const type=detailPage.querySelector('[name=operationType]').value;
...
function purchaseView(c) {
 const t=totals(c);
 open(c.name,`<div class="purchase-detail"><div class="journey-detail-top"><span class="journey-date">${esc(tripDates(c))}</span><button class="chip" data-trip-settings="${esc(c.id)}">Редагувати</button></div><div class="strip">${[['Власним коштом',t.net],['Сплачено',t.paid],['Повернуто',t.recovered]].map(([label,value])=>`<div><span class="label">${label}</span><strong>${euro(value,2)}</strong></div>`).join('')}</div>${c.budget!==null?line('Бюджет',c.budget):''}${c.note?`<details class="journey-notes"><summary>Нотатки</summary><p>${esc(c.note)}</p></details>`:''}<section class="purchase-payments"><div class="section-head"><h2>Оплати</h2><button class="chip" data-payment="${esc(c.id)}">+ Оплата</button></div>${tripDetails(c)}${paymentList(c)}</section></div>`);
...
function collectionEdit(id,kind='trip',editing=false) {
 ...
 open(c.name||'Новий запис',`<form id="collection-form" data-id="${esc(c.id)}" data-reference="${c.referenceAmount??''}">${field('Назва',`<input name="name" value="${esc(c.name)}" maxlength="240" required>`)}${field('Тип',`<select name="kind">${[['trip','Відпустка / подорож'],['event','Подія'],['purchase','Велика покупка']].map(([v,l])=>`<option value="${v}" ${v===c.kind?'selected':''}>${l}</option>`).join('')}</select>`)}<div class="form-pair">${field('Дата початку / покупки',`<input name="start" type="date" value="${c.start}" required>`)}${field('Дата завершення',`<input name="end" type="date" value="${c.end}" required>`)}</div>${field('Бюджет, EUR',`<input data-private name="budget" type="number" min="0" max="10000000" step="0.01" value="${c.budget===null?'':(c.budget/100).toFixed(2)}" placeholder="Ще не визначено">`)}<details class="form-detail"><summary>Додатково</summary>${field('Нотатка',`<textarea name="note" maxlength="2000">${esc(c.note)}</textarea>`)}<label class="checkline"><input name="season" type="checkbox" ${c.season?'checked':''}> Святковий сезон</label></details>${c.referenceAmount!==null?`<p class="quiet">У ручній таблиці: ${euro(c.referenceAmount,2)}. Ця сума сама не створює витрат.</p>`:''}${c.manualPayment?`<details><summary>Ручна оплата</summary><p class="quiet">Не дублюй оплату з виписки.</p>${field('Сплачено вручну, EUR',`<input data-private name="manualAmount" type="number" min="0.01" max="10000000" step="0.01" value="${c.manualPayment?(c.manualPayment.eur/100).toFixed(2):''}" placeholder="Без ручної оплати">`)}${field('Дата ручної оплати',`<input name="manualDate" type="date" value="${c.manualPayment?.date??c.start}">`)}</details>`:paymentList(c)}<details class="trip-link-editor"><summary>Прив’язані оплати</summary><input class="search full-width" id="link-search" type="search" aria-label="Пошук оплат для прив’язки" placeholder="Назва, банк або YYYY-MM"><div id="link-list"></div></details><div class="form-actions"><button class="button">Зберегти</button><button type="button" class="chip" data-action="close">Скасувати</button>${id?`<button type="button" class="link red" data-delete="${esc(id)}">Видалити</button>`:''}</div></form>`);renderLinks('');
}
function renderLinks(search) {
 const rows=allRows().filter(r=>r.source!=='user'&&(selectedRowIds.has(r.id)||(search.trim()&&!r.excluded&&`${r.description} ${r.date} ${r.provider}`.toLocaleLowerCase('uk-UA').includes(search.toLocaleLowerCase('uk-UA'))))).sort((a,b)=>Number(selectedRowIds.has(b.id))-Number(selectedRowIds.has(a.id))||b.date.localeCompare(a.date));
...
async function showHistory() {
 ...
 const labels={cashExpense:'Витрата готівкою',deleteCashExpense:'Видалення готівкової витрати',row:'Назва та категорія операції',categoryName:'Назва категорії',budget:'Ліміт бюджету',collection:'Подія або покупка',deleteCollection:'Видалення запису',undo:'Скасування правки',report_refresh:'Оновлення фінансового звіту',gift_category_split:'Подарунки — окрема категорія'};
