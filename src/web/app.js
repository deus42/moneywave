...
const providerNames={zen:'ZEN',privatbank:'ПриватБанк',wise:'Wise',revolut:'Revolut',erste:'Erste'};
const prettyProvider=v=>providerNames[String(v).toLowerCase()]??v;
...
function tripDetails(c,editable=true) {
 ...
 if(c.kind==='purchase')return `<section class="trip-detail"><p class="quiet purchase-valuation">Усі підсумки — в EUR за оцінкою списань. Початкові суми й ручні ціни збережено в деталях та джерелах.</p><section class="flat-detail trip-payments-detail"><h3 class="flat-title">Оплати та повернення · ${own.length}</h3><ul class="split-breakdown">${own.map(r=>`<li><div><button class="link" ${action(r)}>${esc(r.description)}</button><small>${paymentDate(r.date)} · ${esc(prettyProvider(r.provider))}${r.splitParentId?' · частина переказу':''}</small></div><div><b>${purchaseRowMoney(r)}</b>${nativeAmount(r)&&r.currency!=='EUR'?`<small>Списано ${esc(nativeAmount(r))}</small>`:''}</div></li>`).join('')}</ul></section></section>`;
 return `<section class="trip-detail"><div class="trip-summary">${stats([['Сплачено',euro(t.paid,2)],['Повернено',euro(t.recovered,2)],['Витрати',euro(t.net,2)]])}</div>${c.coverageNote?`<div class="flat-note coverage-note"><span class="flat-label">Частина витрат</span><p>${esc(c.coverageNote)}</p></div>`:''}<div class="trip-types">${t.types.map(x=>`<div><span>${esc(tripTypes[x.type])}</span><strong>${euro(x.eur,2)}</strong></div>`).join('')}</div><p class="quiet">За списаннями: ${euro(t.bankNet,2)}${t.estimatedCount?` · ${t.estimatedCount} оплат в історичній EUR-оцінці`:''}. ${c.kind==='trip'?'Головна сума — ціна покупок після повернень. Різниця з оцінкою списань не є окремою комісією.':''}</p>${t.otherPaid||t.deposits?`<div class="trip-context">${t.otherPaid?`<span>Сплачено іншими <b>${euro(t.otherPaid,2)}</b></span>`:''}${t.deposits?`<span>Депозити / блокування <b>${euro(t.deposits,2)}</b></span>`:''}<small>Поза власними витратами</small></div>`:''}<section class="flat-detail trip-payments-detail"><h3 class="flat-title">Оплати та повернення · ${own.length}</h3><div class="table-wrap"><table class="data trip-payments"><thead><tr><th>Оплата</th><th>Тип</th><th>${c.kind==='purchase'?'EUR':'Ціна EUR'}</th>${c.kind==='purchase'?'':'<th>За списанням</th>'}</tr></thead><tbody>${own.map(r=>`<tr><td><button type="button" ${action(r)}>${esc(r.description)}</button><span class="subline display-block">${paymentDate(r.date)} · ${esc(prettyProvider(r.provider))}</span></td><td>${esc(tripTypes[r.spendingType??'other'])}</td><td class="${r.eur<0?'green':''}">${euro(c.kind==='trip'?r.purchaseValue.eur:r.eur,2)}${r.purchaseValue.estimated&&c.kind==='trip'?'<small class="subline display-block">історична оцінка</small>':''}</td>${c.kind==='purchase'?'':`<td>${euro(r.eur,2)}</td>`}</tr>`).join('')}</tbody></table></div></section></section>`;
}
function chart() {
...
function capitalTrend(c) {
 ...
 if(points.length<2)return '';
 return `<section class="capital-trends" aria-label="Історія капіталу">${capitalTrendSeries(points,'cryptoMinor','Інвестиції','investments')}${capitalTrendSeries(points,'bankMinor','Збереження','savings')}</section>`;
}
function capitalTrendSeries(points,field,title,tone) {
 const values=points.map(p=>Number(p[field])),lo=Math.min(...values),hi=Math.max(...values),span=hi-lo,step=100/(values.length-1),gradient=`ov-area-${crypto.randomUUID()}`;
 const x=i=>i*step,y=v=>span?100-(v-lo)/span*100:50,xy=values.map((v,i)=>`${x(i).toFixed(2)},${y(v).toFixed(2)}`);
 const hits=values.map((v,i)=>{const left=Math.max(0,x(i)-step/2),right=Math.min(100,x(i)+step/2);return `<rect class="ov-hit" x="${left.toFixed(2)}" y="0" width="${(right-left).toFixed(2)}" height="100"><title>${title} · ${date(points[i].asOf)}: ${euro(v)}</title></rect>`;}).join('');
 return `<div class="ov-trend ov-trend-${tone}" data-capital-series="${field}"><div class="ov-trend-head"><h3><i class="ov-dot" aria-hidden="true"></i>${title}</h3><span>${date(points[0].asOf)} — ${date(points.at(-1).asOf)}</span></div><div class="ov-plot"><svg class="ov-line-chart" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="${title}: історія оціненої частини у євро"><defs><linearGradient id="${gradient}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" class="ov-stop-top"/><stop offset="1" class="ov-stop-bottom"/></linearGradient></defs><path class="ov-grid" d="M0,0H100M0,50H100M0,100H100"/><path class="ov-area" fill="url(#${gradient})" d="M0,100L${xy.join('L')}L100,100Z"/><path class="ov-line" d="M${xy.join('L')}"/><path class="ov-end" d="M${xy.at(-1)}l0,0"/>${hits}</svg><div class="ov-y" aria-hidden="true">${plotAxis(hi,lo)}</div></div></div>`;
}
...
function cryptoBox() {
 ...
 const cards=c.crypto.map(p=>{
  ...
  return `<article class="crypto-card"><div class="crypto-top"><h3>${p.chain==='near'?'NEAR':'Ethereum'}</h3><div class="crypto-value">${euro(p.eurMinor,2)}<span>${h?'Історична оцінка':usd(p.usdMinor)}</span></div></div><section class="flat-detail crypto-details"><h3 class="flat-title">Активи та оцінка</h3>${detail}</section><div class="crypto-footer"><span>${h?'Ціна на ':''}${date(p.observedAt)}</span><a href="${esc(h?.sources.find(s=>s.pair!=='EURUSDT')?.sourceUrl??p.sourceUrl)}" target="_blank" rel="noopener noreferrer">${h?'Binance':esc(p.source)} ↗</a></div></article>`;
 }).join('');
...
function collectionToolbar(kind) {
 const options=kind==='purchase'?[['all','Усі покупки'],['matched','З транзакцією'],['unmatched','Без транзакції'],['cash','Сплачено готівкою'],['review','Потребує звірки'],['season','Святкові'],['archived','Архів']]:[['all','Усі'],['trip','Поїздки'],['event','Події'],['season','Святкові']];
...
function collectionSummary(selected,budgetCategory=null) {
 ...
 return `<section class="collection-summary"><div><span>Витрати</span><strong>${euro(total,2)}</strong></div><div><span>Бюджет${planned.length&&!complete?' · '+planned.length+' із '+selected.length:''}</span><strong>${planned.length?euro(budget):'—'}</strong></div>${complete?`<div><span>${total>budget?'Понад бюджет':'Залишилось'}</span><strong class="${total>budget?'red':'green'}">${euro(Math.abs(budget-total))}</strong></div>`:''}</section>`;
}
...
function tripView(c,tab='overview') {
 ...
 const overview=`<div class="journey-overview">${categories}<section class="journey-settlement"><h3>Оплати</h3>${paymentLine('Сплачено',t.paid)}${paymentLine('Повернуто',t.recovered,'green')}${t.otherPaid?paymentLine('Інші платники · поза витратами',t.otherPaid):''}${t.deposits?paymentLine('Депозити · поза витратами',t.deposits):''}<div class="flat-note journey-reconciliation"><span class="flat-label">Банківська оцінка · ${euro(t.bankNet,2)}</span><p>Ціна покупки в EUR; якщо її немає — історична оцінка.${t.estimatedCount?' Таких оплат: '+t.estimatedCount+'.':''} Різниця з банківською оцінкою не є окремою комісією.</p></div></section></div>${c.note?`<div class="flat-note journey-notes"><span class="flat-label">Нотатки</span><p>${esc(c.note)}</p></div>`:''}`;
 ...
 open(c.name,`<div class="journey-detail"><div class="journey-detail-top"><span class="journey-date">${esc(tripDates(c))}</span><button class="chip" data-trip-settings="${esc(c.id)}">Редагувати</button></div><section class="journey-hero"><div class="journey-own"><span>Витрати</span><strong>${t.count?euro(t.net,2):'—'}</strong></div><div class="journey-hero-budget"><span>Бюджет</span>${c.budget===null?`<button data-trip-settings="${esc(c.id)}" data-focus-budget>Додати</button>`:`<strong>${euro(c.budget,2)}</strong><small class="${over?'red':'green'}">${over?'Понад план':'Залишок'} ${euro(Math.abs(c.budget-t.net),2)}</small>`}</div></section>${c.coverageNote?`<div class="flat-note coverage-note"><span class="flat-label">Частина витрат</span><p>${esc(c.coverageNote)}</p></div>`:''}<div class="journey-detail-tabs" role="tablist" aria-label="Деталі поїздки">${[['overview','Огляд'],['payments','Оплати · '+t.count]].map(([value,label])=>`<button id="journey-tab-${value}" role="tab" aria-controls="journey-panel" aria-selected="${tab===value}" tabindex="${tab===value?0:-1}" data-trip-tab="${value}" data-trip-owner="${esc(c.id)}">${label}</button>`).join('')}</div><div id="journey-panel" role="tabpanel" aria-labelledby="journey-tab-${tab}">${tab==='payments'?payments:overview}</div></div>`);
...
function operationSplitView(r){
 const split=workspace.state.operationSplits[r.id],parts=allRows().filter(p=>p.splitParentId===r.id),confirmed=parts.filter(p=>!p.excluded).reduce((n,p)=>n+p.eur,0);
 open(r.description,`<div class="op-summary"><div class="op-amount"><span>Списання · EUR</span><strong>${euro(r.eur,2)}</strong><small>Оригінал: ${cashMoney(-Number(r.nativeMinor),split.currency)} · ${date(r.date)}</small></div></div><h2>Склад оплати</h2><ul class="split-breakdown">${parts.map(p=>`<li><div><strong>${esc(p.description)}</strong><small>${p.unresolved?'Нерозподілено · поза підтвердженими витратами':esc(categoryName(p.group))}</small>${operationPurchaseLinks(p)}</div><div><b>${euro(p.eur,2)}</b><small>Оригінал: ${cashMoney(-Number(p.nativeMinor),split.currency)}</small></div></li>`).join('')}</ul><p>Підтверджені витрати: <b>${euro(confirmed,2)}</b></p><p class="quiet">EUR-оцінку списання розподілено пропорційно. Це не курс фактичної купівлі валюти. Повне списання повторно у витрати не входить.</p>${split.note?`<div class="flat-note"><p>${esc(split.note)}</p></div>`:''}${originalOperationDescription(r)}<div class="form-actions"><button class="button" data-split="${esc(r.id)}">Редагувати розподіл</button></div>`);
}
...
function rowEdit(id) {
 ...
 const report=euro(Math.abs(r.eur),2),day=r.date.length===10?transactionDayLabel(r.date):paymentDate(r.date);
 ...
 open(r.description||'Операція',`<div class="op-summary"><div class="op-amount${r.eur<0?' is-refund':''}"><span>${label}</span><strong>${report}</strong>${native&&r.currency!=='EUR'?`<small>Оригінал: ${esc(native)}</small>`:''}</div><div class="op-facts">${fact('Дата',day)}${fact('Рахунок',prettyProvider(r.provider))}${r.trip?fact('Поїздка',r.trip):''}</div></div>${operationPurchaseLinks(r)}${originalOperationDescription(r)}${canSplit(r)?`<button type="button" class="chip" data-split="${esc(id)}">Розподілити оплату</button>`:''}<form id="row-form" class="op-form" data-id="${esc(id)}">
...
function savingsLine() {
 ...
 if(!s){
  ...
  return '<button class="line total-line ov-savings" id="ov-savings" data-action="savings"><span class="ov-line-label"><span>Заощадження</span></span><strong class="muted">…</strong></button>';
 }
 const b=s.bridge,rest=b?afterFx(b.remainder,b.fx):null;if(!b||rest===null)return '';
 return `<button class="line total-line ov-savings ${b.saved<0?'red':'green'}" id="ov-savings" data-action="savings"><span class="ov-line-label"><span>Заощадження${view.income>0?`<small>${Math.round(b.saved/view.income*100)}% доходу</small>`:''}</span></span><strong>${euro(b.saved)}</strong></button>`;
}
async function showSavings() {
 ...
 const bridge=rest===null?'<p class="quiet">За цей період є лише ручні оплати, тож залишок доходу не розраховується.</p>':
  ...
  +step('Мало б заощадитись',b.expected,'sv-total','',true)
  +step('Заощадження',b.saved,'sv-total','Erste, Wise, Revolut, ZEN, готівка · без крипти',true)
 ...
 const head=`<thead><tr><th>Рахунок</th><th>${date(s.startAsOf)}</th><th>${date(s.endAsOf)}</th><th>Заощаджено</th><th>Переоцінка</th></tr></thead>`;
 ...
 const otherTable=other.length?`<section class="flat-detail sv-other"><h3 class="flat-title">${esc(otherNames)} · поза заощадженнями · зміна ${signed(b.otherAccounts)}</h3><div class="table-wrap"><table class="data sv-accounts">${head}<tbody>${other.map(row).join('')}</tbody></table></div></section>`:'';
 const months=s.months.length>1?`<section class="sv-months"><h2>По місяцях</h2><div class="table-wrap"><table class="data sv-month-table"><thead><tr><th>Місяць</th><th>Залишок доходу</th><th>Заощаджено</th><th>Різниця</th><th>Непояснено</th></tr></thead><tbody>${s.months.map(m=>{const r=afterFx(m.remainder,m.fx),d=r===null?null:m.saved-r;return `<tr><td>${monthNames[Number(m.month.slice(5))-1]} ${m.month.slice(0,4)}</td><td>${money(r)}</td><td>${money(m.saved)}</td><td class="${d===null?'':d<0?'red':'green'}">${signed(d)}</td><td class="muted">${signed(m.unexplained)}</td></tr>`;}).join('')}</tbody></table></div></section>`:'';
 open('Заощадження',`<p class="quiet">${date(s.from)} — ${date(s.to)} · залишки на ${date(s.startAsOf)} і ${date(s.endAsOf)}. Заощаджено — зміна рахунку без переоцінки валют.</p>${stats([['Залишок доходу',money(rest)],['Заощадження',money(b.saved)],['Різниця',signed(gap),gap===null?'':gap<0?'red':'green']])}<section class="sv-bridge"><h2>Звідки різниця</h2>${bridge}${b.revaluation?`<p class="quiet sv-note">Окремо: переоцінка валют ${signed(b.revaluation)}. Це зміна вартості USD у EUR, а не заощадження.</p>`:''}</section><section class="sv-accounts-section"><h2>Рахунки</h2>${accounts}${otherTable}</section>${months}`);
}
...
function paymentList(c) {
 ...
 return `<section class="manual-payments">${payments.length?`<section class="flat-detail"><h3 class="flat-title">Додаткові оплати · ${payments.length}</h3><div class="table-wrap"><table class="data"><tbody>${payments.map(p=>`<tr><td><button type="button" data-payment="${esc(c.id)}" data-payment-id="${esc(p.id)}">${esc(p.description)}</button><small class="subline display-block">${paymentDate(p.date)} · ${p.disposition==='deposit'?'депозит':p.payer==='other'?esc(p.payerName||'Інший платник'):p.linkedRowId?'пов’язано з випискою':'власна оплата'}</small></td><td>${euro(p.eur,2)}</td></tr>`).join('')}</tbody></table></div></section>`:''}${workspace.state.collections.some(x=>x.id===c.id)?`<button type="button" class="chip" data-payment="${esc(c.id)}">+ Оплата</button>`:''}</section>`;
}
