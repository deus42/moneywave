...
export function createWorkspaceServer(options: {store:WorkspaceStore; centers:Pick<FinanceCenters,'capital'|'capitalHistory'>; crypto?:Pick<CryptoStore,'positions'>; now?:()=>Date; staticDirectory:string; sourceCurrent:boolean}) {
  ...
  const server = createServer(async (req,res) => {
    ...
    const cookieName = `moneywave_session_${address.port}`;
